...
do
  ...
  #echo $numofqpt1 > ${OUTPUTPREFIX}$i
   sed -n "$startline, $endline p" $ComboQfile | awk '{printf("%16.9f  %16.9f  %16.9f \n",$1, $2, $3)}' >> ${OUTPUTPREFIX}$i
done
...
##Special treatment of the last file
  ...
  # fi
   sed -n "$startline, \$ p" $ComboQfile | awk '{printf("%16.9f  %16.9f  %16.9f \n", $1, $2, $3)}' >> ${OUTPUTPREFIX}${numoffiles}
else
